...
def make_predict(model, tokenizer, data_dir):
    ...
    all_adj = torch.tensor([f.adj for f in eval_features], dtype=torch.long)
    all_input_mask = torch.tensor([f.input_mask for f in eval_features], dtype=torch.long)
    all_segment_ids = torch.tensor([f.segment_ids for f in eval_features], dtype=torch.long)
    all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
    eval_data = TensorDataset(all_input_ids, all_adj,all_input_mask, all_segment_ids, all_example_index)
    ...
    for input_ids,adj, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating",
        ...
        with torch.no_grad():
            batch_start_logits, batch_end_logits, batch_answer_type_logits = model(input_ids,adj, segment_ids, input_mask)
