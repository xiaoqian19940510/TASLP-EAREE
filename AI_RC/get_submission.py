...
def get_sample_lst():
    mrc_pre = open('pretrained_model/Bert-wwm-ext/predictions.json',encoding='utf-8')
    out_data = json.load(mrc_pre)
    mrc_test = open('AI_RC/data/test.json', "r", encoding="utf-8")
    lst = []
    for data in mrc_test:
        ...
        data['type'] = re.findall("event type (.*?) ?", question)[0]
        data['role'] = re.findall("the role (.*?) of event", question)[0]
...
def merge_type_lst():
    sub_data = open('predict.json','w',encoding='utf-8')
    sample_lst = get_sample_lst()
    official_test_df = open('dataset/dev_base.json','r',encoding='utf-8')
    merge_lst = []
